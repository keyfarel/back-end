...
=======
                ...
                success: function (data) {
                    // console.log('Success Response:', data);
>>>>>>> 595e4cf2cf2f8c636c1c89b09ecf2eda49b95ac2
                    const letterContainer = document.querySelector(".letter-card table tbody");
                    const navElement = document.querySelector('nav[aria-label="Page navigation example"]');
                    const tableHeader = `
                        <thead>
        ...
        keyword.addEventListener('keyup', function() {
            ...
            debounceTimeout = setTimeout(function() {
                $.ajax({
                    ...
                    // Populate table rows with data
                    data.forEach(letter => {
                        const statusBadge = letter.status === 1 ? '<span class="px-3 py-1 text-xs font-medium text-yellow-700 bg-yellow-100 rounded-full">Tertunda</span>' : letter.status === 2 ? '<span class="px-3 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-full">Disetujui</span>' : '<span class="px-3 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-full">Ditolak</span>';
